...
u3 = Contract "3" $ And (american (1 Month, 3 Months) (One $ 10 NZD)) (when (at $ 3 Months) (Give $ One $ 100 USD))
-- main = zcb
...
scaleCal o cal = lift2 (*) o cal
--scaleCal o cal = Obs (\time -> ((getValue cal time)*(getValue o time)))

zipCal :: Calender -> Calender -> Calender
--zipCal cal1 cal2 = Obs (\time -> (if (((getValue cal1 time) /= "") && ((getValue cal2 time) /= "")) then ((getValue cal1 time) ++ "and " ++ (getValue cal2 time)) else (if (((getValue cal1 time) == "") && ((getValue cal2 time) == "")) then "" else ((getValue cal1 time) ++ (getValue cal2 time)))))
--zipCal cal1 cal2 = Obs (\time -> ((getValue cal1 time) + (getValue cal2 time)))
zipCal cal1 cal2 = lift2 (+) cal1 cal2
...
giveCal cal = Obs (\time ->  (-1)*(getValue cal time))
